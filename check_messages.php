...
include "config.php";
    ...
    // $user = $_GET['loggedin'];
    // $last_check = $_GET['last_check'];
    // $query = "SELECT * FROM messages WHERE CURRENT_TIMESTAMP > DATE_SUB(NOW(), INTERVAL 2 SECOND) and receiver_id = $user and seen = 0";
    // $stmt = $mysqli->prepare($query);
    // $stmt->execute();
    // $result = $stmt->get_result();
    // if($result->num_rows > 0) {
    ...
    //     $receiver_id = $row['receiver_id'];
    //     //update the message status to read
    //     $update_query = "UPDATE messages SET seen = 1 WHERE seen = 0 AND time_stamp > CURRENT_TIMESTAMP and receiver_id = $user";
    //     $mysqli->query($update_query);
    //     echo json_encode(array("has_new_message" => true,"receiver_id" => $receiver_id));
    // } else {
    //     echo json_encode(array("has_new_message" => false));
    ...
    if($result->num_rows > 0) {
        ...
        $update_query = "UPDATE messages SET seen = 1 WHERE seen = 0 AND time_stamp > CURRENT_TIMESTAMP and receiver_id = $user";
        $mysqli->query($update_query);
        echo json_encode(array("has_new_message" => true,"MsgCount"=>$result->num_rows ,"receiver_id" => $receiver_id));
    } else {
        echo json_encode(array("has_new_message" => false));
    }